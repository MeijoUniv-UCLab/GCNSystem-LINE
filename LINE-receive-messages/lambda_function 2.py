...
#ボタンメッセージ作成
def createButtonMessages(point_nos):
...
def lambda_handler(event, context):
    ...
    def on_message(line_event):
        ...
        for i, item in enumerate(collection_points):
            ...
            point_no = item['properties']['PointId']
            collection_address.append([i+1, location, lat, lon])
            point_nos.append(point_no)
        # 結果の表示
        for row in collection_address:
            print(row)
        for row in point_nos:
            print(row)
        ...
        #ボタン作成
        location_messages.append(createButtonMessages(point_nos))
    ...
    def on_postback(line_event):
        wait_message = TextSendMessage("しばらくお待ちください")
        LINE_BOT_API.reply_message(line_event.reply_token, wait_message)
        logger.info(line_event)

        data = line_event.postback.data
        try:
            index, gcpid = data.split(':', 1)  # データをインデックスとIDに分割
        except ValueError:
            logger.error("Invalid postback data format: %s", data)
            index = "1"
            gcpid = data
        print(gcpid)
        ...
        GarbageCollectionPointId = gcpid
        GarbageCollectionPointId = int(GarbageCollectionPointId)
        # 結果の表示
        ...
        #登録完了メッセージ
        request_message = TextSendMessage(f"集積所{index}の登録が完了しました。")
        LINE_BOT_API.push_message(LineUserId, request_message)
